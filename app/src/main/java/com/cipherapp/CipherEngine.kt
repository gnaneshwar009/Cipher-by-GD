...
object CipherEngine {
    ...
    fun encrypt(input: String) = buildString { for (ch in input) append(encryptMap[ch] ?: ch) }
    fun decrypt(input: String) = buildString { for (ch in input) append(decryptMap[ch] ?: ch) }
    fun isSupportedChar(ch: Char) = encryptMap.containsKey(ch)
}
